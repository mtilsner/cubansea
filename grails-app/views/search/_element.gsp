<div id="${result.id}" class="result" resultTitle="${result.title.encodeAsHTML()}" resultSummary="${result.summary.encodeAsHTML()}" resultUrl="${result.url}">
	...
	<div id="${result.id}_relevanceSquare" class="relevanceSquare ${collapsed ? "collapsed" : "expanded"}"" title="Similarity: ${Math.round(result.getRelevance(cluster)*100)}%" onclick="javascript:cubansea.toggleSummary('${result.id}')"></div>
